    ...
    <div className="container">
      ...
      <div className="testi-cta">
        <div className="testi-cta-stats">
          {[{val:'4.8/5',label:'Average Rating'},{val:'2,000+',label:'Reviews'},{val:'98%',label:'Would Recommend'}].map((s,i)=>(
            <div key={i} className="tcs-item">
              <strong>{s.val}</strong><span>{s.label}</span>
            </div>
          ))}
        </div>
        <a href="#contact" className="btn-testi-cta" onClick={e=>{e.preventDefault();document.getElementById('contact')?.scrollIntoView({behavior:'smooth'})}}>
          Join WeIntern →
        </a>
      </div>
    </div>
